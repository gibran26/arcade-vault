...
- **`game-performance-booster`** (`.claude/agents/game-performance-booster.md`) — optimiza el rendimiento de rendering de **un juego concreto**, el que se le indique explícitamente, aplicando el catálogo de técnicas ya validado por `specs/11-optimizacion-rendimiento-frogger.md` en `app/game-engines/frogger/engine.ts` (capa estática cacheada, sprites offscreen con el glow horneado, tiles precalculados, variantes discretas por `Record`, reconstrucción de cachés dentro de `setSkin`). Opera sobre un solo motor por invocación: audita su `engine.ts` en busca de `shadowBlur` por primitiva, fondos retrazados cada frame o geometría recalculada, e implementa las correcciones sin tocar física, reglas ni la firma pública del motor. Por defecto usa solo análisis estático; mide con Playwright únicamente si se le pide de forma explícita ("mide FPS antes y después"). Puede tocar el HUD compartido `GamePlayClient.tsx` si lo justifica, verificando que los otros 4 juegos no sufran regresión. Como `skin-designer` y `mobile-porter`, sí escribe y edita código; no mantiene memoria persistente entre corridas.
