...
- [x] "¿Olvidaste tu contraseña?" envía el correo de recuperación; al hacer clic en el enlace, `/auth/callback` redirige a `/auth/update-password`; establecer ahí una nueva contraseña permite iniciar sesión con ella después.
...
- [x] `middleware.ts` refresca la sesión en Server Components/Server Actions sin requerir un refresh manual del navegador durante una sesión larga.
...
- **`middleware.ts` se implementó como `proxy.ts`** (con la función exportada `proxy`, no `middleware`), en vez del nombre de archivo que indicaba el plan original — descubierto durante la verificación manual: Next.js 16 deprecó la convención `middleware.ts`/`export function middleware` en favor de `proxy.ts`/`export function proxy` (confirmado en `node_modules/next/dist/docs/01-app/02-guides/upgrading/version-16.md`), tal como advierte `AGENTS.md` sobre cambios de convención en esta versión. El contenido y patrón interno (`@supabase/ssr`, `getUser()`, cookies, matcher) es idéntico al planeado; solo cambia el nombre del archivo y de la función exportada.
- **Hallazgo durante la verificación del criterio de refresco de sesión: bajar el "JWT expiry" a 60 segundos para forzar la expiración disparó la protección anti-replay de Supabase (`refresh_token_already_used`, "Possible abuse attempt") y revocó la sesión completa (`token_revoked`), en vez de refrescarla.** Se confirmó con los logs de Auth (`get_logs`) que el middleware sí intentaba refrescar correctamente (`token_refreshed` aparece varias veces), pero con un expiry de 60s varias requests casi simultáneas de una misma navegación (RSC payload, prefetch, assets) llegaban al middleware en paralelo y competían por refrescar con el mismo `refresh_token`; la primera ganaba la carrera y rotaba el token, y las siguientes, al llegar con el `refresh_token` ya usado, activaban la detección de reutilización de Supabase, que revoca toda la sesión como medida de seguridad. No es un defecto del middleware, sino un artefacto de la agresividad del valor de prueba elegido. Repetir la prueba con el expiry en 300 segundos (tiempo suficiente para que las requests concurrentes de una navegación terminen antes de la siguiente expiración) confirmó el comportamiento esperado: la sesión se refrescó sin intervención manual y la navegación a `/auth` redirigió a `/games`.
