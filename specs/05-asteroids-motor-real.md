...
- **Campo "Nivel" del HUD**: la interfaz `AsteroidsCallbacks` incluye `onLevelChange?: (level: number) => void` como campo opcional. **Corrección post-implementación (spec 06):** el engine de Asteroids sí maneja niveles internamente (`nextLevel()` aumenta la dificultad y el canvas dibuja `NIVEL n`); la premisa original de que "el juego no tiene niveles" era incorrecta. `initGame()` y `nextLevel()` invocan `onLevelChange(level)`, y `play/page.tsx` lo conecta a `setLevel`, así que el HUD de React ahora sí refleja el nivel real en vez de quedarse fijo en `01`.
...
6. **Conectar pausa real y sincronización de estado** — el botón "PAUSA" llama a `pause()`/`resume()` de la instancia en el `useRef`; el estado visual de pausa en el HUD se actualiza a partir de `onPauseChange(isPaused)` en vez de solo la acción local del botón. **Corrección post-implementación (spec 06):** el campo Nivel del HUD se conecta a `onLevelChange`, que sí es invocado por el engine (ver corrección en el punto "Campo Nivel del HUD" del Alcance). El sistema queda funcional: pausar detiene el juego real y el HUD refleja el estado confirmado por el engine.
...
- [x] ~~El campo "Nivel" del HUD se muestra fijo en `01` para este juego; `onLevelChange` está declarado en el tipo pero nunca es invocado por el engine de Asteroids.~~ **Corrección post-implementación (spec 06):** el engine sí maneja niveles y ahora invoca `onLevelChange(level)`; el campo "Nivel" del HUD refleja el nivel real de la partida.
...
- ~~**`onLevelChange` se define como campo opcional en `AsteroidsCallbacks` pero no se invoca en este spec**, porque el juego original no tiene niveles...~~ **Corrección post-implementación (spec 06):** esta premisa era incorrecta — el `game.js` original y el engine portado sí tienen niveles (`nextLevel()`, dificultad creciente, `NIVEL n` dibujado en el canvas). El campo permanece opcional en el tipo (sigue siendo útil para motores que no tengan niveles), pero el engine de Asteroids ahora sí lo invoca desde `initGame()` y `nextLevel()`.
...
- ~~**Campo "Nivel" fijo en `01`**, en vez de eliminarlo del HUD...~~ **Corrección post-implementación (spec 06):** ya no aplica — el campo ahora refleja el nivel real vía `onLevelChange`.
