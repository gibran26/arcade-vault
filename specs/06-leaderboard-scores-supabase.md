...
- **`Podium` solo se renderiza en `HallOfFameClient.tsx` cuando hay al menos 3 filas**, mostrando un mensaje ("AÚN NO HAY SUFICIENTES PUNTUACIONES PARA EL PODIO") en su lugar cuando hay menos. Encontrado durante la verificación manual del Paso 6: `Podium.tsx` asume `rows[0]`/`rows[1]`/`rows[2]` sin chequeo, algo que nunca fallaba con el mock (`seededScores()` siempre genera ≥12 filas) pero sí con datos reales de Asteroids, que empiezan en 0 puntuaciones y crecen de a una. Se resolvió con un guard en el componente que llama a `Podium`, sin tocar `Podium.tsx`, preservando la decisión de no modificarlo. Decisión explícita del usuario.
